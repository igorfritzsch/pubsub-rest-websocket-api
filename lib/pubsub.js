...
exports.subscribe = function subscribe(dest, con) {
  ...
  if (subscription_exist === false) {
    ...
    if (con !== null) {
      storage.push({'name': destination_pattern, 'data': null, 'sid': SID, 'time': (Date.now() + delsubtime), 'ws': [ con ]});
    }
    else {
      storage.push({'name': destination_pattern, 'data': null, 'sid': SID, 'time': (Date.now() + delsubtime), 'ws': []});
    }
  }
  return {'name': destination_pattern, 'type': 'subscribe', 'response': true};
};
...
exports.publish = function publish(dest, msg){
  ...
  return {'name': destination_pattern, 'type': 'publish', 'response': true};
};
...
exports.getTopic = function getTopic(dest){
  ...
  if (subscription_exist === false) {
    return {'name': destination_pattern, 'type': 'get', 'response': false};
  }
  else {
    return {'name': destination_pattern, 'type': 'get', 'response': true};
  }
};
...
exports.getTopicMessage = function getTopicMessage(dest){
  ...
  if(subscription_exist === false){
    ...
    return {'name': destination_pattern, 'type': 'get', 'response': true, 'message': message};
  }
};
